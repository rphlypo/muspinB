...
function [Exp, glsl] = GratingCreatingUpdating(Exp, win, update)
...
%         Exp.Stimulus.grat_bar1 = Exp.Current.GratingLum(1)-Exp.Visual.Grating(1).standev .* ones(Exp.PTB.w, Exp.Visual.Grating(1).dutycycle * (1 / Exp.Visual.Grating(1).freq));
%         Exp.Stimulus.grat_bar2 = Exp.Current.GratingLum(2)-Exp.Visual.Grating(2).standev .* ones(Exp.PTB.w, Exp.Visual.Grating(2).dutycycle * (1 / Exp.Visual.Grating(2).freq));
        Exp.Stimulus.grat_bar1 = Exp.Current.GratingLum(1) .* ones(Exp.PTB.w, Exp.Visual.Grating(1).dutycycle * (1 / Exp.Visual.Grating(1).freq));
        Exp.Stimulus.grat_bar2 = Exp.Current.GratingLum(2) .* ones(Exp.PTB.w, Exp.Visual.Grating(2).dutycycle * (1 / Exp.Visual.Grating(2).freq));
